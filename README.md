...
- **授权录入群**（`TELEGRAM_ENTRY_CHAT_IDS`）：群内**任意成员**可用（含精准查重菜单、`/查重` `/录入`）。
- **未授权群**：机器人不响应。
...
授权群（`TELEGRAM_ENTRY_CHAT_IDS`）内任意成员可用：
...
流程：
1. 先按用户名/昵称/电话做**辅助查重**（非 ID 百分百）
2. 已存在 → 提示命中，不重复写入
3. 不存在 → 写入**待确认客户**（`P######`），再用「选择客户」或 `/resolve_select` 补 Telegram ID，转为正式客户（ID 精准查重）

群里也可用菜单：选择客户 / 转发消息，直接做 Telegram ID 精准查重录入。
